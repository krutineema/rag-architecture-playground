...
# 13. Next Experiment — Chunking

Experiment 02 should now deliberately make chunking a retrieval variable.

The current baseline is too simple because every document is one chunk.

The next experiment should therefore use the same corpus but split the
documents into multiple chunks and investigate:
...
The important objective is not to discover a universally optimal chunk size.

It is to demonstrate the architectural trade-off:
...
more precise retrieval
    ↓
but potentially less context

Larger chunks
    ↓
more context
    ↓
but potentially more irrelevant content
```

We should also modify the test corpus/questions so that some relevant information
crosses a chunk boundary. That will make the effect of chunking observable rather
than simply producing different chunk counts.
